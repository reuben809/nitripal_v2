...
class Database:
    def __init__(self):
        ...
        # You need to set this in your Streamlit secrets.toml file
        self.connection_string = st.secrets.get("mongodb_uri", "")

        # If not in secrets, use a default connection string (for local development)
        if not self.connection_string:
            # Replace with your own connection string from MongoDB Atlas
            self.connection_string = "mongodb+srv://<username>:<password>@<cluster-url>/nutripal?retryWrites=true&w=majority"
    ...
    def _ensure_indexes(self):
        ...
        # Food logs indexes
        self.food_logs.create_index([("username", 1), ("date", 1)])
        self.food_logs.create_index("created_at")

        # Community recipes indexes
        self.community_recipes.create_index("tags")
        self.community_recipes.create_index("created_at")

        # Analytics indexes
        self.analytics.create_index([("username", 1), ("date", 1)])
    ...
    def get_collection_stats(self):
        ...
        try:
            stats = {
                ...
                "food_logs": self.food_logs.count_documents({}),
                "community_recipes": self.community_recipes.count_documents({}),
                "analytics": self.analytics.count_documents({})
            }
            return stats
        except Exception as e:
